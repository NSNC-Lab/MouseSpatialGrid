...
plot(avg_data_Control,'k','LineWidth',1.5);hold on
plot(avg_data_laser,'r','LineWidth',1.5); hold on
...
xlabel('Time (ms)')
xticks(20:10:60)
xticklabels([50:200:850])
legend('control','laser')

print(gcf,'-vector','-dsvg',['C:\Users\ipboy\Documents\Modeling Paper\Figures\Figure',num2str(plot_num),'\Resubmission2025_2\','ZoomPlot','.svg']) % svg
...
for m = 1:length(FR)/2
    ...
    for j = 1:3
        ...
        for k = 1:10
            ...
            %% Rasters
            %Grab the spikes that we are interested in
            S1OnOff = transpose(data(subz).spks.S1OnOff(m*2-1).channel1(k,:));
            R1on = transpose(data(subz).spks.R1On(m*2-1).channel1(k,:));
            R2on = transpose(data(subz).spks.R2On(m*2-1).channel1(k,:));
...
for m = 1:length(FR)/2
    ...
    for j = 1:3
        ...
        for k = 1:10
            ...
            %% Rasters
            %Grab the spikes that we are interested in
            S1OnOff = transpose(data(subz).spks.S1OnOff(m*2).channel1(k,:));
            R1on = transpose(data(subz).spks.R1On(m*2).channel1(k,:));
            R2on = transpose(data(subz).spks.R2On(m*2).channel1(k,:));
