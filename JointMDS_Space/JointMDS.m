...
function feature_vector = compute_response_features(trial_bins, sample_rate_hz, bin_width, stimulus_event_info)
    ...
    onset_responsiveness = window_responsiveness_index(mean_rate, ...
        clip_indices(stimulus_event_info.first_onset_bins, n_bins_local));
    offset_responsiveness = window_responsiveness_index(mean_rate, ...
        clip_indices(stimulus_event_info.first_offset_bins, n_bins_local));
    ...
    feature_vector = [ ...
        ...
        half_max_width_ms, ...
        temporal_center_ms, ...
        onset_responsiveness, ...
        temporal_sparsity, ...
        trial_reliability, ...
        offset_responsiveness ...
        ];
end
...
function plot_stimulus_event_windows(stimulus_event_info, output_name)
    ...
    onset_mask = zeros(size(x_ms));
    offset_mask = zeros(size(x_ms));
    onset_mask(stimulus_event_info.onset_bins) = 1;
    offset_mask(stimulus_event_info.offset_bins) = 1;

    f = figure('Position', [50, 50, 1200, 700]);
    t = tiledlayout(2, 1, 'Padding', 'compact', 'TileSpacing', 'compact');

    ax1 = nexttile(t, 1);
    hold(ax1, 'on')
    shade_mask_regions(ax1, x_ms, onset_mask, [0.85, 0.92, 1.00], 1.05);
    shade_mask_regions(ax1, x_ms, offset_mask, [1.00, 0.90, 0.85], 1.05);

    plot(ax1, x_ms, y, 'k', 'LineWidth', 1.8)
    xline(ax1, x_ms(stimulus_event_info.start_idx), '--', 'Start');
    xline(ax1, x_ms(stimulus_event_info.end_idx), '--', 'End');
    xline(ax1, x_ms(stimulus_event_info.first_peak_idx), '--', '1st peak');
    xline(ax1, x_ms(stimulus_event_info.first_trough_idx), '--', '1st trough');
    ylabel(ax1, 'Norm. envelope')
    title(ax1, 'Stimulus envelope with all onset and offset regions')
    ylim(ax1, [0, 1.05])
    box(ax1, 'on')
    grid(ax1, 'on')
    ...
    ax2 = nexttile(t, 2);
    hold(ax2, 'on')
    stairs(ax2, x_ms, onset_mask, 'Color', [0.15, 0.45, 0.85], 'LineWidth', 2.0)
    stairs(ax2, x_ms, offset_mask, 'Color', [0.85, 0.35, 0.15], 'LineWidth', 2.0)
    xline(ax2, x_ms(stimulus_event_info.start_idx), '--', 'Start');
    xline(ax2, x_ms(stimulus_event_info.end_idx), '--', 'End');
    xline(ax2, x_ms(stimulus_event_info.first_peak_idx), '--', '1st peak');
    xline(ax2, x_ms(stimulus_event_info.first_trough_idx), '--', '1st trough');
    xlabel(ax2, 'Time (ms)')
    ylabel(ax2, 'Mask weight')
    title(ax2, 'Exact onset/offset bins used for responsiveness across the full stimulus')
    ylim(ax2, [-0.05, 1.15])
    yticks(ax2, [0, 1])
    legend(ax2, {'Onset bins', 'Offset bins'}, 'Location', 'northoutside', 'Orientation', 'horizontal')
    box(ax2, 'on')
    grid(ax2, 'on')

    sgtitle(t, 'Regions used for onset and offset responsiveness')
    exportgraphics(f, output_name, 'ContentType', 'vector')
end

function shade_mask_regions(ax, x_ms, mask_values, fill_color, max_height)
    mask_values = mask_values(:).';
    padded_mask = [0, mask_values, 0];
    start_points = find(diff(padded_mask) == 1);
    end_points = find(diff(padded_mask) == -1) - 1;

    for region_idx = 1:numel(start_points)
        region_start = start_points(region_idx);
        region_end = end_points(region_idx);
        patch(ax, ...
            [x_ms(region_start), x_ms(region_end), x_ms(region_end), x_ms(region_start)], ...
