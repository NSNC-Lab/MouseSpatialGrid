...
function feature_vector = compute_response_features(trial_bins, sample_rate_hz, bin_width, stimulus_event_info)
    ...
    onset_responsiveness = window_responsiveness_index(mean_rate, onset_window);
    offset_responsiveness = window_responsiveness_index(mean_rate, offset_window);
...
function plot_stimulus_event_windows(stimulus_event_info, output_name)
    ...
    if ~isempty(onset_x)
        patch([onset_x(1), onset_x(end), onset_x(end), onset_x(1)], [0, 0, 1.05, 1.05], ...
              [0.85, 0.92, 1.00], 'EdgeColor', 'none', 'FaceAlpha', 0.35, 'Parent', ax1);
    end

    if ~isempty(offset_x)
        patch([offset_x(1), offset_x(end), offset_x(end), offset_x(1)], [0, 0, 1.05, 1.05], ...
              [1.00, 0.90, 0.85], 'EdgeColor', 'none', 'FaceAlpha', 0.35, 'Parent', ax1);
    end
    ...
    title(ax1, 'Stimulus envelope with first onset and first offset regions')
    ylim(ax1, [0, 1.05])
    box(ax1, 'on')
    grid(ax1, 'on')
    ...
    title(ax2, 'Exact onset/offset bins used for responsiveness')
    ylim(ax2, [-0.05, 1.15])
    yticks(ax2, [0, 1])
