...
for i=source_labels
    for j=masker_labels
        ...
        if i == j
            ...
            annotation('textarrow', x_normalized, y_normalized, 'String', 'Source + Masker', 'FontSize', 12, 'Color', 'blue');
        else
            if i ~= 0
                xx = pre_normalized_x(i) + 1;
                % Convert data coordinates to normalized figure units
                x_normalized = ([xx, xx] - xlim(1)) / (xlim(2) - xlim(1));
                y_normalized = ([3.5,2.75] - ylim(1)) / (ylim(2) - ylim(1));
                annotation('textarrow', x_normalized, y_normalized, 'String', 'Source', 'FontSize', 12, 'Color', 'green');
            end
            if j ~= 0
                ...
                x_normalized = ([xx, xx] - xlim(1)) / (xlim(2) - xlim(1));
                y_normalized = ([3.5,2.75] - ylim(1)) / (ylim(2) - ylim(1));
                annotation('textarrow', x_normalized, y_normalized, 'String', 'Masker', 'FontSize', 12, 'Color', 'red');
            end
        end                                    
