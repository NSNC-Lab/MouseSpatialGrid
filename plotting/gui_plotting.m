...
function gui_plotting(TD_SOMs, TD_SOMt, inhibs_XRs, inhibs_XRt, inhibs_PEs, inhibs_PEt, targets, sources, Nodesx, Nodesy, SOM_nodes, all_gsyns, arrow_sizes, reverseNodeMap)
    % Create the main figure
    % structure_data = load('curr_data_structure.mat');
    fig = figure('Position', [100, 100, 400, 300], 'MenuBar', 'none', 'Name', 'Select Locations', 'NumberTitle', 'off', 'Resize', 'off');
    
    % Create the text labels
    uicontrol('Style', 'text', 'Position', [50, 240, 50, 20], 'String', 'Source:');
    uicontrol('Style', 'text', 'Position', [50, 160, 50, 20], 'String', 'Masker:');
    ...
    function select_i(val)
        selected_i = val;
        disp(['Selected source: ', num2str(selected_i)]);
    ...
    function select_j(val)
        selected_j = val;
        disp(['Selected masker: ', num2str(selected_j)]);
