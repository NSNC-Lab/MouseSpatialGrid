...
POINT_SELECTION_MODE = "all_final_batches"
...
POINT_SELECTION_MODE_LABELS = {
    "best_final_batch": "best final-epoch batch per cell",
    "all_final_batches": "all final-epoch batch fits",
}
POINT_SELECTION_MODE_SLUGS = {
    "best_final_batch": "best_final_batch",
    "all_final_batches": "all_final_batches",
}
...
def validate_point_selection_mode(selection_mode: str) -> None:
    if selection_mode not in POINT_SELECTION_MODE_LABELS:
        allowed = ", ".join(sorted(POINT_SELECTION_MODE_LABELS))
        raise ValueError(
            f"Unsupported POINT_SELECTION_MODE={selection_mode!r}. Allowed values: {allowed}"
        )


def point_selection_mode_label(selection_mode: str) -> str:
    validate_point_selection_mode(selection_mode)
    return POINT_SELECTION_MODE_LABELS[selection_mode]


def point_selection_mode_slug(selection_mode: str) -> str:
    validate_point_selection_mode(selection_mode)
    return POINT_SELECTION_MODE_SLUGS[selection_mode]
...
def build_analysis_dataframe(
    ...
    validate_point_selection_mode(POINT_SELECTION_MODE)
    ...
    unit_count = min(len(all_data), params.shape[2], losses.shape[2], best_batch_indices_1based.size)
    batch_count = min(params.shape[3], losses.shape[3])

    param_cols = build_parameter_names(params.shape[1])
    layer_values = extract_layer_labels(all_data, unit_count)
    final_epoch_binned_sse = losses[-1, 1, :unit_count, :batch_count]

    records: list[dict[str, object]] = []
    for unit_idx in range(unit_count):
        best_batch = int(best_batch_indices_1based[unit_idx])
        if POINT_SELECTION_MODE == "best_final_batch":
            batch_indices_1based = [best_batch]
        else:
            batch_indices_1based = range(1, batch_count + 1)

        for batch_index_1based in batch_indices_1based:
            batch_idx = batch_index_1based - 1
            if batch_idx < 0 or batch_idx >= batch_count:
                continue

            param_values = np.asarray(params[epoch_idx, :, unit_idx, batch_idx], dtype=float)
            record: dict[str, object] = {
                "unit_id": unit_idx + 1,
                "batch_index": int(batch_index_1based),
                "best_batch": best_batch,
                "is_best_batch": bool(batch_index_1based == best_batch),
                "layer": layer_values[unit_idx],
                "final_binned_sse": float(final_epoch_binned_sse[unit_idx, batch_idx]),
            }
            for col_name, value in zip(param_cols, param_values):
...
def build_model_hover_raster_previews(
    output: np.ndarray,
    unit_ids_1based: np.ndarray,
    batch_indices_1based: np.ndarray,
) -> list[str]:
    previews: list[str] = []
    for unit_id, batch_index in zip(unit_ids_1based, batch_indices_1based):
        try:
...
def save_parameter_pairplot(
    ...
    palette = palette_for_layers(layer_labels)
    point_count = len(df)
    scatter_size = 48 if point_count <= 800 else 18 if point_count <= 2500 else 10
    scatter_alpha = 0.85 if point_count <= 800 else 0.45 if point_count <= 2500 else 0.22
    scatter_edgecolor = "white" if point_count <= 1500 else None
    scatter_linewidth = 0.4 if point_count <= 1500 else 0.0
    ...
    if len(param_cols) == 1:
        ...
        ax.set_title(
            f"Parameter distribution by cortical layer ({point_selection_mode_label(POINT_SELECTION_MODE)})"
        )
        fig.tight_layout()
    ...
    grid.figure.suptitle(
        "Parameter pairplot colored by cortical layer "
        f"({point_selection_mode_label(POINT_SELECTION_MODE)})",
        y=1.02,
    )
...
def save_seaborn_embedding_plot(
    ...
    palette = palette_for_layers(layer_labels)
    point_count = len(df)
    scatter_size = 70 if point_count <= 800 else 32 if point_count <= 2500 else 18
    scatter_alpha = 0.9 if point_count <= 800 else 0.55 if point_count <= 2500 else 0.28
    ...
    ax.set_title(
        "UMAP projection of parameters colored by cortical layer "
        f"({point_selection_mode_label(POINT_SELECTION_MODE)})"
    )
...
def save_bokeh_hover_embedding(
    ...
    <div style="width: 360px;">
      ...
      <div><strong>Batch:</strong> @batch_index</div>
      <div><strong>Best batch:</strong> @best_batch</div>
      <div><strong>Is best batch:</strong> @is_best_batch</div>
      <div><strong>Final binned SSE:</strong> @final_binned_sse{0.000000}</div>
    ...
    plot = figure(
        title=(
            "UMAP projection with target raster previews on hover "
            f"({point_selection_mode_label(POINT_SELECTION_MODE)})"
        ),
        width=950,
        height=760,
...
def save_bokeh_hover_embedding_with_model_and_topography(
    ...
    hover_df["model_raster_png"] = build_model_hover_raster_previews(
        output=output,
        unit_ids_1based=hover_df["unit_id"].to_numpy(dtype=int),
        batch_indices_1based=hover_df["batch_index"].to_numpy(dtype=int),
    )
    ...
    <div style="width: 690px;">
      ...
      <div style="display: flex; gap: 12px; margin-top: 8px;">
        ...
        <div>
          <div style="font-weight: 600; margin-bottom: 4px;">Sim raster for this batch</div>
          <img
    ...
    plot = figure(
        title=(
            "UMAP projection with layer topography and data/model raster hover "
            f"({point_selection_mode_label(POINT_SELECTION_MODE)})"
        ),
        width=950,
        height=760,
...
def main() -> None:
    ...
    validate_point_selection_mode(POINT_SELECTION_MODE)
    selection_mode_label = point_selection_mode_label(POINT_SELECTION_MODE)
    selection_mode_slug = point_selection_mode_slug(POINT_SELECTION_MODE)
    ...
    print(f"Building analysis table using {selection_mode_label}.")
    ...
    print(
        "Prepared "
        f"{len(analysis_df)} plotted points across "
        f"{analysis_df['unit_id'].nunique()} cells."
    )

    csv_path = OUTPUT_DIR / f"{selection_mode_slug}_params_with_layers_and_umap.csv"
    analysis_df.to_csv(csv_path, index=False)
    print(f"Saved analysis table to: {csv_path}")

    pairplot_path = OUTPUT_DIR / f"{selection_mode_slug}_param_pairplot_by_layer.png"
    ...
    embedding_path = OUTPUT_DIR / f"{selection_mode_slug}_umap_embedding_by_layer_seaborn.png"
    ...
    if MAKE_BOKEH_HOVER:
        ...
        hover_path = OUTPUT_DIR / f"{selection_mode_slug}_umap_embedding_by_layer_hover.html"
        ...
        hover_topography_path = (
            OUTPUT_DIR
            / f"{selection_mode_slug}_umap_embedding_by_layer_hover_with_model_and_topography.html"
        )
