% Fitness function
function fitness = fitness_fun_tar90_maskNeg90(Target_grid,Target_fr_grid, varied_params, optionsGA, plot_all, toggle_real)
    ...
    varied_struct.XR1 = varied_params(5);
    varied_struct.XR2 = varied_params(6);
    varied_struct.XR3 = varied_params(7);
...
%0.00175685264443262	0.000198679983008015	0.00229808130178778	0.00149230691029864
    ...
    %Make sure we do not have SOM rebounding
    fitness1 = 0;

    for vz = 1:length(varied_params)
        if varied_params(vz) > 0.01
            fitness1 = fitness1 + 100;
        end
    end
    ...
    if fr_grid(1,3) > 30
        fitness2 = fitness2 + (fr_grid(1,3) - 30)*20;
    end
