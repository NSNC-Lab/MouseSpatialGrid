...
varies(end).range = [1.5 2];

varies(end+1).conxn = 'R->C';
varies(end).param = 'gSYN';
varies(end).range = [0.16 0.13];
...
subplot('position',posVec)
plot([-90 0 45 90],dataFR(targetIdx),[-90 0 45 90],fr.CT,'linewidth',2);
legend('Data','Model');
xlabel('Azimuth');
ylabel('Clean FR (Hz)')
set(gca,'xdir','reverse');
...
% saveas(gca,[filesep DirPart filesep 'clean FR ' mat2str(rcNetcon) '_' num2str(vv) '.png']);
