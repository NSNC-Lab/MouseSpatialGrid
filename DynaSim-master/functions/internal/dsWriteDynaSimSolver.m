function [outfile,options] = dsWriteDynaSimSolver(model,netcons,varargin)
    ...
    if options.save_parameters_flag
      ...
      if ndims_per_var(i)==1
        ...
        if isequal(sizes_per_var{i},pop_size)
          ...
          %1. See if we can just change 1 state variable to initalize
          %differently.
          %2. See if we can get it to run with size 2
          %3. See if we can make it dynamic
...
fprintf(fid,'  t=T(k-1);\n');
%fprintf(fid,'dbstop in solve_ode_4_channel_PV_inputs at 592;\n');
if options.downsample_factor==1 && options.disk_flag==0 % store every time point, do not use var_last
  ...
  print_statevariables_again_update(fid,state_variables)
  fprintf(fid,'  n=n+1;\n');
  if options.sim_log_flag
...
function print_var_update(fid,index_nexts,index_lasts,update_term,state_variables)
  ...
  %IB Added to reduce memory?
  % for i=1:length(state_variables)
  % 
  %   state_variables{i} = strrep(state_variables{i}, 'n-1', '1');
  ...
  for i=1:length(conditionals)
    ...
    for j=1:length(state_variables)
      if strcmp('spike_monitor',conditionals(i).namespace)
        ...
        %IB Brought in so that we can change it in the else statement
...
%       end
    ...
    for j=1:length(condition)
      ...
      if ~isempty(indCondStr)
        ...
        %Temp commented out. Seems like these were causing issues
