...
%Adjusting E-> As it says in paper for fig nine to compensate****IMPORTANT
%Nominally 0.02
...
varies(end).range = [0.1;0.1];

% E->PV connections
%varies(end+1).conxn = '(On->S1OnOff,Off->S1OnOff,R1On->S2OnOff,R1Off->S2OnOff)';
%varies(end).param = '(gSYN,tauP)';
%varies(end).range = [0.032;80];

%0.2 fp = baseline E->PV
%fp = 1 means completely inhibited
%Offset -> PV connections
varies(end+1).conxn = '(Off->S1OnOff)';
varies(end).param = '(gSYN,fP,tauP)';
varies(end).range = [0.005;0;80];

%Offset 2
varies(end+1).conxn = '(R1Off->S2OnOff)';
varies(end).param = '(gSYN,fP,tauP)';
varies(end).range = [0.005;0;80];

%Onset -> PV connections
varies(end+1).conxn = '(On->S1OnOff)';
varies(end).param = '(gSYN,fP,tauP)';
varies(end).range = [0.020;0.2;80];

%Onset2
varies(end+1).conxn = '(R1On->S2OnOff)';
varies(end).param = '(gSYN,fP,tauP)';
varies(end).range = [0.020;0.2;80];
