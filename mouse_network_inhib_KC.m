...
Imask = zeros(1,nCells);
s.populations(end+1).name='TD';
s.populations(end).equations = 'LIF_Iapp';
s.populations(end).size = 1;
s.populations(end).parameters = {'Itonic',0,'noise',0,'Imask',Imask'};
...
s.connections(end).mechanism_list={'synDoubleExp'};
s.connections(end).parameters={'gSYN',0.08, 'tauR',0.3, 'tauD',1.5, 'netcon', eye(nCells)}; 
