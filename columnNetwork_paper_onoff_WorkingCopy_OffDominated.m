...
s.connections(end).parameters={'gSYN',0.02,'tauR',EE_rise,'tauD',EE_fall,'fP',0.5,'tauP',30};
...
s.connections(end).mechanism_list={'PSC'};
s.connections(end).parameters={'gSYN',0.02,'tauR',EE_rise,'tauD',EE_fall,'fP',0,'tauP',30};
% 
...
simdata = dsSimulate(s,'tspan',[dt time_end], 'solver',solverType, 'dt',dt,...
  ...
  'mex_flag',options.mex_flag);%,'parfor_flag',1,'num_cores',24);
toc;
