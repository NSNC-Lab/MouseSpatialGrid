...
from BuildFile import Forwards_Method_cupy_recovery, calculate_loss_all_cells
import numpy as np
import time
...
CHECKPOINT_PREFIX = "checkpoint_Eprop_All_cells_recovery"
TOTAL_EPOCHS = 10
#RESUME_CHECKPOINT_PATH = "C:\\Users\\ipboy\\Documents\\GitHub\\ModelingEffort\\Multi-Channel\\Plotting\\OliverDataPlotting\\checkpoint_Eprop_All_cells_recovery_epoch_0005.mat"  # Example: r"checkpoint_Eprop_All_cells_latest.mat"
...
GENERATED_RECOVERY_SOLVER_PATH = Path(__file__).resolve().parent / "BuildFile" / "generated_solve_file_recovery.py"
...
def build_recovery_solver(solve_file_body, out_path=GENERATED_RECOVERY_SOLVER_PATH):
    ...
    module_name = "generated_solve_file_recovery"
...
class runSimulation(object):
    ...
    file_body_forwards = Forwards_Method_cupy_recovery.Euler_Compiler(arch[0],arch[1],arch[2],opts,num_cells)
    ...
    for epoch in range(start_epoch, TOTAL_EPOCHS):
        ...
        output, grads, on_track, off_track, loss_holder = generated_solve_file.solve_run(on_spks,off_spks,noise,rate_on,rate_off,rate_on_deriv,rate_off_deriv,data,p,recovery_funcs) #Python Verison to build
    ...
    savemat(
        f"output_compressed_Eprop_All_cells_recovery_{timestamp}.mat",
        {
            "output": output,
            "losses": losses,
