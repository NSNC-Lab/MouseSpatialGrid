...
CHECKPOINT_PREFIX = "checkpoint_Eprop_All_cells_recovery_learn_refractory"
TOTAL_EPOCHS = 35
RESUME_CHECKPOINT_PATH = "C:\\Users\\ipboy\\Documents\\GitHub\\ModelingEffort\\Multi-Channel\\Plotting\\OliverDataPlotting\\checkpoint_Eprop_All_cells_recovery_learn_refractory_epoch_0010.mat"  # Example: r"checkpoint_Eprop_All_cells_latest.mat"
...
GENERATED_RECOVERY_SOLVER_PATH = Path(__file__).resolve().parent / "BuildFile" / "generated_solve_file_recovery_learn_refractory.py"
...
REFRACTORY_PSTH_WEIGHT = 0.50
REFRACTORY_ISI_HIST_WEIGHT = 1.00
REFRACTORY_ISI_MEDIAN_WEIGHT = 0.25
...
def precompute_data_isi_reference(data_rasters, bin_edges):
    ...
    return {"hist": hist, "median_steps": median_steps, "n_isis": n_isis}
...
def score_refractory_candidates(output, data_isi_reference, isi_bin_edges, psth_loss):
    output = np.asarray(output)
    psth_loss = np.asarray(psth_loss, dtype=np.float32)
    num_cells, batch_size = psth_loss.shape
    ...
    psth_denominator = np.nanmedian(psth_loss, axis=1)
    psth_denominator[~np.isfinite(psth_denominator) | (psth_denominator <= 0)] = 1.0
    psth_norm = psth_loss / psth_denominator[:, None]
    ...
    for cell_idx in range(num_cells):
        ...
        for batch_idx in range(batch_size):
            ...
            scores[cell_idx, batch_idx] = (
                REFRACTORY_PSTH_WEIGHT * psth_norm[cell_idx, batch_idx]
...
def build_recovery_solver(solve_file_body, out_path=GENERATED_RECOVERY_SOLVER_PATH):
    ...
    module_name = "generated_solve_file_recovery_learn_refractory"
...
def save_training_checkpoint(
    ...
    optional_payload = {
        ...
        "best_psth_loss_per_cell": best_psth_loss_per_cell,
    }
...
class runSimulation(object):
    ...
    best_psth_loss_per_cell = np.full((num_cells,), np.nan, dtype=np.float32)
    ...
    if RESUME_CHECKPOINT_PATH:
        ...
        if checkpoint_best_state:
            ...
            best_psth_loss_per_cell = np.asarray(
                checkpoint_best_state.get("best_psth_loss_per_cell", best_psth_loss_per_cell),
                dtype=np.float32,
            ).reshape(num_cells)
    ...
    for epoch in range(start_epoch, TOTAL_EPOCHS):
        ...
        if np.any(improved_cells):
            ...
            best_psth_loss_per_cell[improved_idx] = psth_loss[improved_idx, chosen_batch_idx]
    ...
    savemat(
        f"output_compressed_Eprop_All_cells_recovery_learn_refractory_{timestamp}.mat",
        {
            "output": output,
            "losses": losses,
