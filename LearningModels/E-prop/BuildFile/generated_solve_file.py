...
def solve_run(on_input,off_input,noise_token,data,p):
    ...
    for timestep,t in enumerate(np.arange(0,29801*0.1-0.1,0.1)):
        ...
        psc_wrt_spiking_On_ROn = ( 0.1*(On_ROn_scale*((On_ROn_PSC_x[:,:,:,-1] + On_ROn_PSC_q[:,:,:,-1])*(1-(np.tanh(t-(On_tspike[:,:,:,-1]+On_ROn_PSC_delay)))**2)))  )*100 
        psc_wrt_spiking_On_SOnOff = ( 0.1*(On_SOnOff_scale*((On_SOnOff_PSC_x[:,:,:,-1] + On_SOnOff_PSC_q[:,:,:,-1])*(1-(np.tanh(t-(On_tspike[:,:,:,-1]+On_SOnOff_PSC_delay)))**2)))  )*100 
        psc_wrt_spiking_Off_SOnOff = ( 0.1*(Off_SOnOff_scale*((Off_SOnOff_PSC_x[:,:,:,-1] + Off_SOnOff_PSC_q[:,:,:,-1])*(1-(np.tanh(t-(Off_tspike[:,:,:,-1]+Off_SOnOff_PSC_delay)))**2)))  )*100 
        psc_wrt_spiking_SOnOff_ROn = ( 0.1*(SOnOff_ROn_scale*((SOnOff_ROn_PSC_x[:,:,:,-1] + SOnOff_ROn_PSC_q[:,:,:,-1])*(1-(np.tanh(t-(SOnOff_tspike[:,:,:,-1]+SOnOff_ROn_PSC_delay)))**2)))  )*100 
    ...
    grads = np.sum(np.stack([spike_wrt_gsyn_Off_SOnOff], axis = 0), axis = 2)
