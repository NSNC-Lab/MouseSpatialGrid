"""CuPy variant of the binned E-prop gradient code generator.

This module emits Python source-code snippets that are intended to run with
CuPy available as `cp` inside the generated solve file.
"""
...
def compileGrad(neurons,synapses,projections,options):
    ...
    return phi_declaration,eligbility_declaration,Bk_declaration, grad_declaration, loss_declration, return_declaration
...
def compile_spiking_wrt_odeVoltage(neurons):
    ...
    for k in neurons:
        ...
        #spiking_deriv += f'        tspike_wrt_odeVoltage_{neuron_name} = (t - {neuron_name}_tspike[:,:,:,-1])*(1/(1+cp.exp({neuron_name}_V[:,:,:,-2]-{neuron_name}_V_thresh)))*(cp.exp(-({neuron_name}_V[:,:,:,-1]-{neuron_name}_V_thresh))/(1+cp.exp(-({neuron_name}_V[:,:,:,-1]-{neuron_name}_V_thresh)))**2)\n'
        #spiking_deriv += f'        print(cp.shape(tspike_wrt_odeVoltage_{neuron_name}))\n'
...
def compile_odeVoltage_wrt_spiking(synapses,options):
    ...
    for k in synapses:
        ...
        #voltage_wrt_psc += f'        voltage_wrt_psc_{synapse_name} = {post_node}_R * ({synapse_name}_gSYN[:,None,None]*cp.dot(({post_node}_V[:,:,:,-2]-{synapse_name}_ESYN),{synapse_name}_netcon))/{post_node}_tau\n'

        #voltage_wrt_psc += f'        voltage_wrt_psc_{synapse_name} = ( {post_node}_R * ({synapse_name}_gSYN*cp.dot(({post_node}_V[:,:,:,-2]-{synapse_name}_ESYN).reshape(cp.shape({post_node}_V[:,:,:,-2])[0]*cp.shape({post_node}_V[:,:,:,-2])[1],cp.shape({post_node}_V[:,:,:,-2])[2]),{synapse_name}_netcon).reshape(cp.shape({post_node}_V[:,:,:,-2])[0],cp.shape({post_node}_V[:,:,:,-2])[1],cp.shape({post_node}_V[:,:,:,-2])[2]))/{post_node}_tau  ) / 10\n' #10 is normalization component
       

        #voltage_wrt_psc += f'        voltage_wrt_psc_{synapse_name} = (-{post_node}_R * {synapse_name}_gSYN*{synapse_name}_netcon*({post_node}_V[:,:,:,-1]-{synapse_name}_ESYN))/{post_node}_tau\n'

        voltage_wrt_psc += f'        voltage_wrt_psc_{synapse_name} = (-{post_node}_R  * {synapse_name}_gSYN*{synapse_name}_netcon*({post_node}_V[:,:,:,-1]-{synapse_name}_ESYN))/{post_node}_tau/10\n'

        #(((cp.tanh(t-({pre_node}_tspike[:,:,:,-1]+{synapse_name}_PSC_delay)))))

        #voltage_wrt_psc += f'        print(cp.shape(voltage_wrt_psc_{synapse_name}))\n'

        #1. Reshape to be cp.dot blas-compatilbe
        #2. Do the dot product
        #3. Revert back to the previous shape
        
        #Example usage of blas-compatible line
        #cp.dot(({post_node}_V[:,:,:,-2]-{synapse_name}_ESYN).reshape(cp.shape({post_node}_V[:,:,:,-2])[0]*cp.shape({post_node}_V[:,:,:,-2])[1],cp.shape({post_node}_V[:,:,:,-2])[2]),{synapse_name}_netcon).reshape(cp.shape({post_node}_V[:,:,:,-2])[0],cp.shape({post_node}_V[:,:,:,-2])[1],cp.shape({post_node}_V[:,:,:,-2])[2])
    ...
    for k in synapses:
        synapse_name = k['name']
        pre_node = k['name'].rsplit('_', 1)[0]
        #psc_wrt_spiking += f'        psc_wrt_spiking_{synapse_name} = {options["dt"]}*({synapse_name}_scale*({synapse_name}_PSC_x[:,:,:,-2] + ((-{synapse_name}_PSC_q[:,:,:,-2]*(-cp.exp(t-({pre_node}_tspike[:,:,:,-1]+{synapse_name}_PSC_delay)))))/(1+cp.exp(t-({pre_node}_tspike[:,:,:,-1]+{synapse_name}_PSC_delay)))**2) - {synapse_name}_PSC_s[:,:,:,-1])/{synapse_name}_tauR\n'
        #psc_wrt_spiking += f'        print((1+cp.exp(t-({pre_node}_tspike[:,:,:,-1]+{synapse_name}_PSC_delay)))**2)\n'
        #psc_wrt_spiking += f'        print(t)\n'
        #psc_wrt_spiking += f'        print({pre_node}_tspike[:,:,:,-1])\n'

        #Try stable version
        #psc_wrt_spiking += f'        psc_wrt_spiking_{synapse_name} = {options["dt"]}*({synapse_name}_scale*({synapse_name}_PSC_x[:,:,:,-2] + {synapse_name}_PSC_q[:,:,:,-2]*(1/(cp.cosh(t-({pre_node}_tspike[:,:,:,-1]+{synapse_name}_PSC_delay))))**2))\n'
        #Stable version v2 replace 1/cosh with 1-tanh
        psc_wrt_spiking += f'        psc_wrt_spiking_{synapse_name} = ( {options["dt"]}*({synapse_name}_scale*(({synapse_name}_PSC_q[:,:,:,-1])*2*(cp.tanh(t-({pre_node}_tspike[:,:,:,-1]+{synapse_name}_PSC_delay)))*(1-(cp.tanh(t-({pre_node}_tspike[:,:,:,-1]+{synapse_name}_PSC_delay)))**2)))  )*100 \n' #100 is normalization component (positively?)
        #psc_wrt_spiking += f'        psc_wrt_spiking_{synapse_name} = ( {options["dt"]}*({synapse_name}_scale*(({synapse_name}_PSC_x[:,:,:,-1] + {synapse_name}_PSC_q[:,:,:,-1])*((cp.tanh(t-({pre_node}_tspike[:,:,:,-1]+{synapse_name}_PSC_delay))))))  )*100 \n' #100 is normalization component (positively?)

        #psc_wrt_spiking += f'        print(cp.shape(psc_wrt_spiking_{synapse_name}))\n'
...
def compile_odeVoltage_wrt_gsyn(synapses):
    ...
    for k in synapses:
        ...
        # if synapse_name == 'On_SOnOff' or synapse_name == 'Off_SOnOff':
        #     voltage_wrt_gsyn += f'        print(\'{synapse_name}\')\n'
        #     voltage_wrt_gsyn += f'        print({synapse_name}_PSC_s[:,:,:,-1])\n'

        #c++ version
        #voltage_wrt_gsyn += f'        voltage_wrt_gsyn_{synapse_name} = {post_node}_R * ({synapse_name}_PSC_s[:,:,:,-1]*cp.dot(({post_node}_V[:,:,:,-1]-{synapse_name}_ESYN).reshape(cp.shape({post_node}_V[:,:,:,-1])[0]*cp.shape({post_node}_V[:,:,:,-1])[1],cp.shape({post_node}_V[:,:,:,-1])[2]),{synapse_name}_netcon).reshape(cp.shape({post_node}_V[:,:,:,-1])[0],cp.shape({post_node}_V[:,:,:,-1])[1],cp.shape({post_node}_V[:,:,:,-1])[2]))/{post_node}_tau\n'


        #voltage_wrt_gsyn += f'        print(cp.shape(voltage_wrt_gsyn_{synapse_name}))\n'


        # - SOnOff_R*(On_SOnOff_gSYN*On_SOnOff_PSC_s[:,:,:,-1]*On_SOnOff_netcon*(SOnOff_V[:,:,:,-1]-On_SOnOff_ESYN)
...
def compile_odeVoltage_wrt_fP(synapses,options):
    ...
    for k in synapses:
        ...
        voltage_wrt_fP += f'        voltage_wrt_fP_{synapse_name} = ({options["dt"]}**2)*(-{post_node}_R * {synapse_name}_gSYN * {synapse_name}_scale*{synapse_name}_PSC_F[:,:,:,-1]*{synapse_name}_PSC_P[:,:,:,-1]*((1-(cp.tanh(t-({pre_node}_tspike[:,:,:,-1]+{synapse_name}_PSC_delay))))**2)*{synapse_name}_netcon*({post_node}_V[:,:,:,-1]-{synapse_name}_ESYN))/({post_node}_tau*{synapse_name}_tauR)\n'
...
def compile_odeVoltage_wrt_tauP(neurons,synapses,options):
    ...
    for k in synapses:
        ...
        post_node = k['name'].rsplit('_', 1)[1]
        pre_node = k['name'].rsplit('_', 1)[0]
        voltage_wrt_tauP += f'        voltage_wrt_tauP_{synapse_name} = (-{post_node}_R*{synapse_name}_gSYN*{synapse_name}_scale*  (     (   {synapse_name}_PSC_F[:,:,:,-1]*(-(1-{synapse_name}_PSC_P[:,:,:,-1])/{synapse_name}_tauP**2)*(1-{synapse_name}_PSC_fP))   *    (cp.tanh(-(t-({pre_node}_tspike[:,:,:,-1]+{synapse_name}_PSC_delay)))  +  1)/2 )   *       0.1*{synapse_name}_netcon*({post_node}_V[:,:,:,-1]-{synapse_name}_ESYN))/({synapse_name}_tauR*{post_node}_tau)\n'
        ...
        #voltage_wrt_tauP += f'        print((cp.tanh(-(t-({pre_node}_tspike[:,:,:,-1]+{synapse_name}_PSC_delay)))  +  1)/2 )\n'
...
def compile_odeVoltage_wrt_g_inc(neurons,synapses,options):
    ...
    for k in neurons:
        ...
        if k['name'] == 'On':
            ...
            neuron_name = k['name']
            voltage_wrt_g_inc += f'        voltage_wrt_g_inc_{neuron_name} = (-{neuron_name}_R*(((1+cp.tanh({neuron_name}_V[:,:,:,-1]-{neuron_name}_V_thresh))/2)*((1+cp.tanh(-({neuron_name}_V[:,:,:,-2]-{neuron_name}_V_thresh)))/2))*0.1*({neuron_name}_V[:,:,:,-1]-{neuron_name}_E_k))/{neuron_name}_tau\n'
...
#     loss_statement += f'    data = cp.transpose(data,(2,0,1))[None,:,:,:]\n'                                  #Rearange the data to match the shape that the network outputs
#     loss_statement += f'    ROn_spikes_holder = cp.transpose(ROn_spikes_holder,(2,0,1,3))\n'                                  #Rearange the data to match the shape that the network outputs
#     loss_statement += f'    num_bins, remainder = divmod(cp.shape(data)[-1], bin_width)\n'              #find the number of bins we would use given a bin width of ex. 200 = 20ms
...
#     #New version
#     loss_statement += f'    forwards_out_reshaped = forwards_out_r.reshape((cp.shape(forwards_out_r)[0]*cp.shape(forwards_out_r)[1],cp.shape(forwards_out_r)[2],num_bins,bin_width))\n'
#     loss_statement += f'    data_reshaped = data_r.reshape((cp.shape(data_r)[0]*cp.shape(data_r)[1],cp.shape(data_r)[2],num_bins,bin_width))\n'
#     #Old version
#     #loss_statement += f'    forwards_out_reshaped = forwards_out_r.reshape((cp.shape(forwards_out_r)[0],cp.shape(forwards_out_r)[1],cp.shape(forwards_out_r)[2],num_bins,bin_width))\n'
#     #loss_statement += f'    data_reshaped = data_r.reshape((cp.shape(data_r)[0],cp.shape(data_r)[1],cp.shape(data_r)[2],num_bins,bin_width))\n'
#     loss_statement += f'    forwards_out_hist = cp.sum(cp.sum(forwards_out_reshaped,axis=-1),axis=-2)\n'

#     #Now we need to pop back up and reinstate our batch dimention.

    

#     loss_statement += f'    data_hist = cp.sum(cp.sum(data_reshaped,axis=-1),axis=-2)\n'


#     loss_statement += f'    forwards_out_hist = forwards_out_hist.reshape(cp.shape(forwards_out_r)[0],cp.shape(forwards_out_r)[1],num_bins)\n'
#     loss_statement += f'    data_hist = data_hist.reshape(cp.shape(data_r)[0],cp.shape(data_r)[1],num_bins)\n'

#     loss_statement += f'    diff = forwards_out_hist - data_hist\n'

#     #loss_statement += f'    print(cp.shape(diff))\n'


#     loss_statement += f'    PSTH_loss_avg = cp.sum(diff * diff, axis=-1)\n'

#     #loss_statement += f'    print(PSTH_loss_avg)\n'

#     loss_statement += f'    PSTH_deriv_avg = 2.0 * cp.sum(diff, axis=-1)\n'
#     loss_statement += f'    out_grad = PSTH_deriv_avg[:,:,None] *cp.sum(grads, axis=-2)\n'
#     #loss_statement += f'    print(cp.shape(out_grad))\n'
#     loss_statement += f'    print(cp.mean(PSTH_deriv_avg))\n'

#     #loss_statement += f'    print(cp.shape(PSTH_deriv_avg))\n'

#     #loss_statement += f'    print(cp.shape(PSTH_deriv_avg* cp.sum(grads, axis=-2)))\n'

#     #loss_statement += f'    print(cp.shape(PSTH_deriv_avg* cp.squeeze(cp.sum(grads, axis=-2))))\n'

#     #loss_statement += f'    print(PSTH_loss_avg)\n'

#     #loss_statement += f'    print(cp.shape(PSTH_deriv_avg))\n'
#     #loss_statement += f'    print(cp.shape(grads))\n'
...
def depth_recursion(neuron,synapses,path,super_path):
    ...
    if not found_child:
        # Order-preserving unique (equivalent to NumPy unique + stable index order).
        super_path.append(list(dict.fromkeys(path)))
...
def fetch_loss(synapses):
    ...
    for k in synapses:
        ...
        if post_node == 'SOnOff':
            fetch_loss_declaration += f'            grad_{synapse_name} += grad_{synapse_name}_accumulate*Bk*loss_deriv[:,:,None,None]\n'
        else:
            fetch_loss_declaration += f'            grad_{synapse_name} += grad_{synapse_name}_accumulate*loss_deriv[:,:,None,None]\n'
   
        fetch_loss_declaration += f'            grad_{synapse_name}_accumulate = 0\n'
...
def compute_eligibility(synapses):
    ...
    for k in synapses:
        ...
        #python version
        eligibility_delcaration += f'        eligibility_{synapse_name} = 0.1*(psi_{post_node}*(-{post_node}_R) * {synapse_name}_PSC_s[:,:,:,:,-1]*{synapse_name}_netcon*({post_node}_V[:,:,:,:,-1]-{synapse_name}_ESYN))/{post_node}_tau\n'

    eligibility_delcaration += f'        eligibility_strf_gain = ((psi_On * (-On_R) * On_g_postIC * 30 * (1 - cp.tanh(0.0001*rate_on[timestep,:,:][:,:,None,None]-1.5)**2) * On_netcon * (On_V[:,:,:,:,-1]-On_E_exc)) / On_tau) + ((psi_Off * (-Off_R) * Off_g_postIC * 30 *(1 - cp.tanh(0.0001*rate_off[timestep,:,:][:,:,None,None]-1.5)**2) * Off_netcon * (Off_V[:,:,:,:,-1]-Off_E_exc)) / Off_tau)\n'
    eligibility_delcaration += f'        eligibility_strf_latency = ((psi_On * (-On_R) * On_g_postIC * 30 *(1 - cp.tanh(0.0001*rate_on_deriv[timestep,:,:][:,:,None,None]-1.5)**2) * On_netcon * (On_V[:,:,:,:,-1]-On_E_exc)) / On_tau) + ((psi_Off * (-Off_R) * Off_g_postIC * 30 *(1 - cp.tanh(0.0001*rate_off_deriv[timestep,:,:][:,:,None,None]-1.5)**2) * Off_netcon * (Off_V[:,:,:,:,-1]-Off_E_exc)) / Off_tau)\n'
    eligibility_delcaration += f'        eligibility_output_adaptation = (-ROn_R*(((1+cp.tanh(ROn_V[:,:,:,:,-1]-ROn_V_thresh))/2)*((1+cp.tanh(-(ROn_V[:,:,:,:,-2]-ROn_V_thresh)))/2))*0.1*(ROn_V[:,:,:,:,-1]-ROn_E_k))/ROn_tau\n'


    #1. Remember that this should be in 0.1ms timescale not ordinal/index 

    eligibility_delcaration += f'        eligibility_ref_ta += -(1-cp.tanh((t-ROn_tspike[:,:,:,:,-1])-ta[:,:,None,None])**2)\n'
    eligibility_delcaration += f'        eligibility_ref_a += (1-cp.tanh(a[:,:,None,None]*(t-ROn_tspike[:,:,:,:,-1])-b[:,:,None,None])**2)*c[:,:,None,None]*(t-ROn_tspike[:,:,:,:,-1])*cp.tanh((t-ROn_tspike[:,:,:,:,-1])-ta[:,:,None,None])\n'
    eligibility_delcaration += f'        eligibility_ref_b += (1-cp.tanh(a[:,:,None,None]*(t-ROn_tspike[:,:,:,:,-1])-b[:,:,None,None])**2)*c[:,:,None,None]*(-1)*cp.tanh((t-ROn_tspike[:,:,:,:,-1])-ta[:,:,None,None])\n'
    eligibility_delcaration += f'        eligibility_ref_c += cp.tanh((t-ROn_tspike[:,:,:,:,-1])-ta[:,:,None,None])*cp.tanh(a[:,:,None,None]*(t-ROn_tspike[:,:,:,:,-1])-b[:,:,None,None]) + cp.tanh((t-ROn_tspike[:,:,:,:,-1])-ta[:,:,None,None])\n'


    return eligibility_delcaration
...
def compute_Lt():
    ...
    Lt_declaration += '        L_t = ROn_mask.astype(cp.float64) - data[:,timestep,:].reshape(1,10,1)\n'
    Lt_declaration += '        L_t_S_SOnOff_ROn = Bk*(ROn_mask.astype(cp.float64) - data[:,timestep,:].reshape(1,10,1))\n'
...
def compile_updates(local_graph,local_graph_synapses,synapses,neurons,VwrtGsyn_declaration=''):
    ...
    for m in synapses: 
        ...
        #parameter_updates += f'        print(cp.shape(spiking_wrt_voltage_{post_node}*voltage_wrt_gsyn_{synapse_name}))\n'
        parameter_updates += f'        spike_wrt_gsyn_{synapse_name}_accumulate[:,:,:,timestep%loss_bin_width] = spiking_wrt_voltage_{post_node}*voltage_wrt_gsyn_{synapse_name}\n'

        # if synapse_name == 'On_SOnOff' or synapse_name == 'Off_SOnOff':
        #     parameter_updates += f'        print(\'{synapse_name}\')\n'
        #     parameter_updates += f'        print(spike_wrt_gsyn_{synapse_name})\n'
...
def compile_return(synapses,neurons,VwrtGsyn_declaration=''):
    ...
    #return_statement = '\n    print(cp.shape(spike_wrt_gsyn_Off_SOnOff))\n'
    #return_statement = '\n    print(cp.shape(cp.stack([spike_wrt_gsyn_Off_SOnOff], axis = 0)))\n'
    ...
    if len(VwrtGsyn_declaration) > 0:  
        ...
        return_statement = return_statement[:-1]
        return_statement += ',eligibility_ref_ta,eligibility_ref_a,eligibility_ref_b,eligibility_ref_c], axis = 0), axis = 3)\n\n'
