...
def declare_vars(neurons,synapses,options, num_cells):
    ...
    variable_declaration += f'\n        raise ValueError(f"recovery_funcs first dimension must match num_cells={num_cells}, got {{recovery_funcs.shape[0]}}")'
    
    variable_declaration += f'\n    print(recovery_funcs.shape[1])'

    variable_declaration += f'\n    recovery_funcs = cp.clip(recovery_funcs, 0.0, 1.0)'
    variable_declaration += f'\n    recovery_max_steps = recovery_funcs.shape[1]'
    # variable_declaration += f'\n    if not getattr(solve_run, "_plotted_recovery_preview", False):'
    # variable_declaration += f'\n        try:'
    # variable_declaration += f'\n            import matplotlib.pyplot as plt'
    # variable_declaration += f'\n            recovery_preview = cp.asnumpy(recovery_funcs[0])'
    # variable_declaration += f'\n            recovery_preview_x_ms = np.arange(recovery_preview.shape[0]) * {options["dt"]}'
    # variable_declaration += f'\n            plt.figure("Recovery function preview")'
    # variable_declaration += f'\n            plt.clf()'
    # variable_declaration += f'\n            plt.plot(recovery_preview_x_ms, recovery_preview, linewidth=2)'
    # variable_declaration += f'\n            plt.xlim(0, 100)'
    # variable_declaration += f'\n            plt.xlabel("Time since last spike (ms)")'
    # variable_declaration += f'\n            plt.ylabel("Recovery probability")'
    # variable_declaration += f'\n            plt.title("Recovery function preview: cell 1")'
    # variable_declaration += f'\n            plt.tight_layout()'
    # variable_declaration += f'\n            plt.show(block=False)'
    # variable_declaration += f'\n            plt.pause(0.001)'
    # variable_declaration += f'\n        except Exception as exc:'
    # variable_declaration += f'\n            print(f"Could not plot recovery function preview: {{exc}}")'
    # variable_declaration += f'\n        solve_run._plotted_recovery_preview = True'
...
def declare_condtionals(neurons,synapses,options):
    ...
    for k in neurons:
        ...
        conditionals_declaration += f'\n        {neuron_name}_recovery_prob = recovery_funcs[recovery_cell_index, {neuron_name}_recovery_idx]'
