...
def solve_run(on_input,off_input,noise_token,rate_on,rate_off,rate_on_deriv,rate_off_deriv,data,p,recovery_funcs):
    ...
    if recovery_funcs.ndim == 2:
        if recovery_funcs.shape[0] != 220:
            raise ValueError(f"2D recovery_funcs first dimension must match num_cells=220, got {recovery_funcs.shape[0]}")
        recovery_funcs = cp.repeat(recovery_funcs[:, None, :], 5, axis=1)
    elif recovery_funcs.ndim == 3:
        if recovery_funcs.shape[0] != 220 or recovery_funcs.shape[1] != 5:
            raise ValueError(f"3D recovery_funcs must have shape (220, 5, steps), got {recovery_funcs.shape}")
    else:
    ...
    recovery_batch_index = cp.arange(5).reshape(1, 5, 1, 1)
    On_C = 0.1
    On_g_L = 0.005
    On_E_L = -65
    On_noise = 0
    ...
    for timestep,t in enumerate(cp.arange(0,29801*0.1-0.1,0.1)):
        ...
        On_recovery_prob = recovery_funcs[recovery_cell_index, recovery_batch_index, On_recovery_idx]
        ...
        Off_recovery_prob = recovery_funcs[recovery_cell_index, recovery_batch_index, Off_recovery_idx]
        ...
        SOnOff_recovery_prob = recovery_funcs[recovery_cell_index, recovery_batch_index, SOnOff_recovery_idx]
        ...
        ROn_recovery_prob = recovery_funcs[recovery_cell_index, recovery_batch_index, ROn_recovery_idx]
