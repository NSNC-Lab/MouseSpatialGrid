from BuildFile import Gradient_Method, Gradient_Method_Binned, Gradient_Method_Binned_cupy_Refractory
...
def Euler_Compiler(neurons,synapses,projections,options, num_cells):
    ...
    phi_declaration,eligbility_declaration,Bk_declaration, grad_declaration,loss_declaration, return_declaration_grad = Gradient_Method_Binned_cupy_Refractory.compileGrad(neurons,synapses,projections,options)
...
def declare_condtionals(neurons,synapses):
    ...
    for k in neurons:
        ...
        if neuron_name == 'ROn':
            # For absolute refractory period clamp the voltage and prevent spiking downstream.
            conditionals_declaration += f'\n        {neuron_name}_mask_2b = cp.any((t <= ({neuron_name}_tspike + ta[:,:,None,None,None])), axis=-1)'
        ...
        #------------------------------------------------#
        if neuron_name == 'ROn':
            # For absolute relative refractory period, allow voltage to vary but reduce the spiking probaility according to the recovery function
            conditionals_declaration += f'\n        {neuron_name}_mask_1 = ((({neuron_name}_V[:,:,:,:,-1] >= {neuron_name}_V_thresh) & ({neuron_name}_V[:,:,:,:,-2] < {neuron_name}_V_thresh)) & (cp.random.random({neuron_name}_tspike[:,:,:,:,-1].shape) < (c[:,:,None,None]*cp.tanh(a[:,:,None,None]*(t-{neuron_name}_tspike[:,:,:,:,-1])-b[:,:,None,None])+c[:,:,None,None])/2)).astype(cp.int8)'
        else:
