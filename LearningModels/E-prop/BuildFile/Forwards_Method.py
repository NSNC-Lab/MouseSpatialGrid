...
def declare_holders(neurons, synapses, options):
    ...
    for k in neurons:
        ...
        if k["is_output"] == 1:
            ...
            holder_declaration += f'\n    On_SOnOff_PSC_s_holder = np.zeros(({options["N_batch"]},{options["N_trials"]},{options["N_channels"]},{options["sim_len"]}), dtype=np.int8)'
            holder_declaration += f'\n    Off_SOnOff_PSC_s_holder = np.zeros(({options["N_batch"]},{options["N_trials"]},{options["N_channels"]},{options["sim_len"]}), dtype=np.int8)'
            holder_declaration += f'\n    losses_holder = np.zeros(({options["N_batch"]},1,{options["N_channels"]},{options["sim_len"]}), dtype=np.int8)'
