...
class runSimulation(object):
    ...
    #Create Dense Parameter Set 5D
    # Define 16 values for each of your 5 parameters
    param1_values = np.linspace(0, 0.02, 16)  # or any 16 values
    param2_values = np.linspace(0, 0.01, 16)
    param3_values = np.linspace(0, 0.03, 16)
    param4_values = np.linspace(0, 0.03, 16)
    param5_values = np.linspace(0.05, 0.09, 16)

    # Generate all combinations (16^5 = 1,048,576 rows)
    all_combinations = np.array(list(itertools.product(
        param1_values, param2_values, param3_values, param4_values, param5_values
    )))

    # Result is a (1048576, 5) matrix
    ...
    for epoch in range(64):
        ...
        p = all_combinations[epoch*batch_size:(epoch+1)*batch_size,:].T  # Shape (5, batch_size)
    ...
    savemat(f"output_compressed_Eprop_{timestamp}.mat", {"losses":losses, "params" : param_tracker,  "best_loss" : np.asarray(best_loss, dtype=np.float32),"best_output" : np.asarray(best_output, dtype=np.float32),"best_params" : np.asarray(best_params, dtype=np.float32)}, do_compression=True)
