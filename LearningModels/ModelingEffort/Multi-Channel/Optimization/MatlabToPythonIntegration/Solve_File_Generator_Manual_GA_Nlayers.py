...
def build_ODE(parameters,batch_size,layer_name):
    ...
    #for name, value in parameters.items():
        ...
        #elif "fP" in name and  f"R{num_layers}Off" not in name:
        
        #    params += f"    {name} = ps[{count_ps}]\n"
        #    count_ps += 1

        #if "t_ref" in name and "On_On_IC_t_ref" not in name and "Off_Off_IC_t_ref" not in name and  f"R{num_layers}Off" not in name:
            #print('here')
        #    params += f"    {name} = ps[{count_ps}]\n"
           

        #elif f"R{num_layers}On_R{num_layers}On_iNoise_V3_FR" in name:
        #    params += f"    {name} = ps[{count_ps}]\n"
        #    count_ps += 1
    ...
    for k in range(len(statement_pairs)):
        ...
        #if f"R{num_layers}Off" not in var_fP:
    ...
    return_statement = f"\n    return R{num_layers}On_V_spikes_holder"
    ...
    with open(f"generated_GA_{layer_name}.py", "w") as f:
        f.write(generated_code)

    print(f"generated_GA_{layer_name}.py has been created.")
