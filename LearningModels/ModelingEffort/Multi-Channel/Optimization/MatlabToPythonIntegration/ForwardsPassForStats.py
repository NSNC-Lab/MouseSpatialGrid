...
def run():
    ...
    p = Init_Params_flexible.pinit(
    batch_size, 10, "generated_GA_Layer4",
    "C:\\Users\\ipboy\\Documents\\GitHub\\ModelingEffort\\Multi-Channel\\Optimization\\MatlabToPythonIntegration\\results\\run_2025-10-02_19-03-18.mat")
    ...
    for epoch in range(GA_Epochs):
        ...
        #print(np.shape(loss))
        print(f"Min l2 PSTH loss = {np.min(loss[1])}",flush=True) 
    ...
    #Grid search for best gain initialization
    p = Init_Params_flexible.pinit(
    batch_size, 19, "generated_GA_Layer23",
    "C:\\Users\\ipboy\\Documents\\GitHub\\ModelingEffort\\Multi-Channel\\Optimization\\MatlabToPythonIntegration\\results\\run_2025-10-02_21-17-42.mat")
    ...
    for epoch in range(GA_Epochs):
        ...
        with mp.get_context("spawn").Pool(processes=n_procs) as pool:
            args_iterable = [(k, p, scale_factor,"generated_GA_Layer23") for k in range(trials)]
            results = pool.starmap(_single_trial, args_iterable)
    ...
    #Grid search for best gain initialization
    p = Init_Params_flexible.pinit(
    batch_size, 28, "generated_GA_Layer56",
    "C:\\Users\\ipboy\\Documents\\GitHub\\ModelingEffort\\Multi-Channel\\Optimization\\MatlabToPythonIntegration\\results\\run_2025-10-03_00-57-41.mat")
    ...
    for epoch in range(GA_Epochs):
        ...
        with mp.get_context("spawn").Pool(processes=n_procs) as pool:
            args_iterable = [(k, p, scale_factor,"generated_GA_Layer56") for k in range(trials)]
            results = pool.starmap(_single_trial, args_iterable)
