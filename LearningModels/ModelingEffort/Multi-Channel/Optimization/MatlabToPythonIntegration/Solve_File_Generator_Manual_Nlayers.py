...
def build_ODE(parameters,batch_size,learning_mask,layer_name):
    ...
    file_path = os.path.join(home_dir, "Documents", "GitHub", "ModelingEffort", 
                         ...
                         "Model-Main", "run", layer_name, "solve", f"solve_ode_{layer_name}.m") #This could be changed for TD and multichannel and what not.
    ...
    for k in range(len(lhs_list)):
        if learning_mask[1] == 1:
            if "tau" in lhs_list[k] and f'R{num_layers}Off' not in lhs_list[k]:
    ...
    for k in range(len(update_vars)):
        if "_V" in update_vars[k] and  "Noise" not in update_vars[k] and f"R{num_layers}Off" not in update_vars[k]:
            ...
            monitor_string += f"    dv2_d{var_name}_tref = np.ones(({batch_size},2))\n"
    ...
    #Declare Inputs
    inputs_header = f"\n    #Delcare Inputs\n    On_On_IC_input = genPoissonInputs.gen_poisson_inputs(trial_number,On_On_IC_locNum,On_On_IC_label,On_On_IC_t_ref,On_On_IC_t_ref_rel,On_On_IC_rec,scale_factor)\n    Off_Off_IC_input = genPoissonInputs.gen_poisson_inputs(trial_number,Off_Off_IC_locNum,Off_Off_IC_label,Off_Off_IC_t_ref,Off_Off_IC_t_ref_rel,Off_Off_IC_rec,scale_factor,\"{layer_name}\")\n"
    ...
    for k in range(len(update_vars)):
        if "_V" in update_vars[k] and  "Noise" not in update_vars[k] and f"R{num_layers}Off" not in update_vars[k]:
            ...
            grad_string += f'        dv1d{var_base}_holder.append({var_base}[:,-1])\n'
    ...
    for name, value in parameters.items():
        if "t_ref" in name and f"R{num_layers}Off" not in name and "On_On" not in name and "Off_Off" not in name:
            ...
            grad_string += f'        dv2_d{cell_name}_tref[:,0] = dv2_d{cell_name}_tref[:,1]\n'
    ...
    if layer_name == 'Layer4':
        ...
        nodes = ['R3On','R2On','R1On','On','Off','S3OnOff','S2OnOff','S1OnOff','R1Off','R2Off','R3Off']
        edges = ['On->R1On','On->S1OnOff','Off->R1Off','Off->S1OnOff','S1OnOff->R1On','S1OnOff->R1Off','R1On->R2On','R1On->S2OnOff','R1Off->R2Off','R1Off->S2OnOff','S2OnOff->R2On','S2OnOff->R2Off','R2On->R3On','R2On->S3OnOff','R2Off->R3Off','R2Off->S3OnOff','S3OnOff->R3On','S3OnOff->R3Off']
    ...
    #
    all_paths = recurse(f'R{num_layers}On',[],nodes,edges)
    ...
    for name, value in parameters.items():
        ...
        if "gSYN" in name and f"R{num_layers}Off" not in name:
    ...
    if learning_mask[1] == 1:
        ...
        if layer_name == 'Layer4' or layer_name == 'Layer4v2':
            ...
            nodes2 = ['On','Off','R1On','R1Off','S1OnOff','R2On','R2Off','S2OnOff','R3On','S3OnOff']
    ...
    for k in nodes2: 
        ...
        t_ref_grads += f'        dtref_{k} = {grad_str_tref}\n'
    ...
    for k in range(len(update_vars)):
        if "_V" in update_vars[k] and ("R" in update_vars[k] or "S" in update_vars[k]) and "Noise" not in update_vars[k] and f"R{num_layers}Off" not in update_vars[k]:
            var = update_vars[k]
            var_base = var[:-3]
    ...
    #print_statement += f'    print(np.min(np.array(dv1dOn_V_holder)[:,0]))\n'
