...
function [meanY, E, correctArray] = calcpc(distMat, numTrials, numTargets, tempNum, numPerms, ver)
...
for ii = 1:numDistMats
    tempMat = distMat(:,:,ii);
	for versionNum = 1:numVersions
		% reset(RandStream.getDefaultStream,0); % this can have negative effects on the code that calls it
		compInds = floor(rand(numPerms*numTargets, 1)*numTrials);
		tempInds = floor(rand(numPerms*numTargets, numTargets)*numTrials);
		tempInds(:,1) = mod(compInds + ceil(rand(numPerms*numTargets, 1)*(numTrials - 1)), numTrials);
		compInds = compInds + 1;
		compShiftSingle = inds2 + (versionNum - 1)*numTrials;
		tempShiftSingle = circulant(inds2) + (tempNum - 1)*numTrials;
		compShift = compShiftSingle(inds1).';
		tempShift = tempShiftSingle(inds1,:);
		a = compInds + compShift;
		for ti = 1:numTargets
			b = a + (tempInds(:,ti) + tempShift(:,ti))*n;
			distances(:,ti) = tempMat(b);
		end
