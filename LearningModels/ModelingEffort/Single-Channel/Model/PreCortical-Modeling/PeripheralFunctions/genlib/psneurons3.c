...
{
    double **rmgsPse, **rmgsPsi, *rmgsPsein, *rmgsPsiin, **V, **rmgse, **rmgsi, **noise, **polee, **polei, **taue, **taui;
    double *arp, *taum, *dtOtaum, *Ese, *Esi, *Vap, *Vth, *Vre, *noiselev, *El, *delay;
    double dt, *Vi, *Vim1, *count, outType, tempPSPe, tempPSPi;
    ...
    if(nrhs < 16 || nrhs > 17 || nlhs > 2) {
        mexErrMsgTxt("16 or 17 inputs and at most 2 outputs required.");
    }
	...
	taui = (double**)mxCalloc(Nn,sizeof(double*));
    polee = (double**)mxCalloc(Nn,sizeof(double*));
    polei = (double**)mxCalloc(Nn,sizeof(double*));
    ...
    /* 16: taui (Nn or Nn x Nn) */
    tempint = mxGetM(prhs[15])*mxGetN(prhs[15]);
    if (!mxIsDouble(prhs[15])|| mxIsComplex(prhs[15]) || ((Nn != mxGetM(prhs[7]) || Nn != mxGetN(prhs[7])) && 1 != tempint ))
        mexErrMsgTxt("Error 16: taui must be a real double NnxNn matrix or scalar");
    ...
    taui[0] = scalarOrMat2Mat(prhs[15],Nn*Nn);
    ...
    /* Convert ARP to #-of-samples; put in minAPdist; combine dt & taums; convert taue and taui to poles */
    if(taum[0] <= 0) mexErrMsgTxt("Error 09: taum <= 0");
    if(taue[0] <= 0) mexErrMsgTxt("Error 15: taue <= 0");
    if(taui[0] <= 0) mexErrMsgTxt("Error 16: taui <= 0");
    ...
    polee[0] = (double*)mxCalloc(Nn*Nn,sizeof(double));
    polei[0] = (double*)mxCalloc(Nn*Nn,sizeof(double));
    ...
    for(ni=1;ni<Nn; ni++) {
        ...
        taui[ni] = taui[ni-1] + Nn;
        polee[ni] = polee[ni-1] + Nn;
        polei[ni] = polei[ni-1] + Nn;
    ...
    maxexplen = 1;
    /* Initialize polee and polei variables */
    for(ni=0;ni<Nn;ni++) {
        for(oi=0;oi<Nn;oi++) {
            if(taue[ni][oi] <= 0) {
                polee[ni][oi] = 0;
            ...
            if(taui[ni][oi] <= 0) {
                ...
                polei[ni][oi] = exp(-dt/taui[ni][oi]);
            }
            
            explen[ni][oi] = max(ceil(8*taue[ni][oi]/dt),ceil(8*taui[ni][oi]/dt));
    ...
    for(ti=1; ti<Ns; ti++) {
                    ...
                    if(polee[ni][oi]) {
                        ...
                        if(startint < Ns) {
                            ...
                            for(tt=1;tt<tempint;tt++) {
                                tempPSPe *= polee[ni][oi];
                                tempPSPi *= polei[ni][oi];
                                rmgsPse[oi][startint+tt] += rmgse[ni][oi]*tempPSPe;
                                rmgsPsi[oi][startint+tt] += rmgsi[ni][oi]*tempPSPi;
                            }
                        }
                    }
                }
        ...
        for(ni=0; ni<Nn; ni++) {
            if(V[ni][tim1] == Vap[ni])
                ...
                V[ni][ti] = max(V[ni][tim1] +
                dtOtaum[ni]*(
                rmgsPse[ni][ti]*(Ese[ni] - V[ni][tim1]) +
                rmgsPsi[ni][ti]*(Esi[ni] - V[ni][tim1]) +
                El[ni] - V[ni][tim1] + noiselev[ni]*noise[ni][ti] )
                , Esi[ni]);
                if(V[ni][ti] >= Vth[ni]) {
                    if( ti >= tinext[ni] ) {
