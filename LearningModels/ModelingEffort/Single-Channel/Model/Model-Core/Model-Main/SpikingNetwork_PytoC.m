...
params_pytoc;                                                     %Parameter file for Onset Dominated Regieme in 2025 Modeling Paper.
...
[snn_out,s] = columnNetwork_pytoc(study_dir,varies,options,flag_raised_mex);  %\TODO Comment and clean
...
all_spikes = [];
for k = 1:10
    all_spikes = [all_spikes,snn_out(k).On_V_spikes];
end
...
all_spikes = [];
for k = 1:10
    all_spikes = [all_spikes,snn_out(k).S1OnOff_V_spikes];
end
