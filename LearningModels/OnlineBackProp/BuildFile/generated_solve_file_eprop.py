...
def solve_run(on_input,off_input,noise_token,data,p):
    ...
    for timestep,t in enumerate(np.arange(0,29801*0.1-0.1,0.1)):
        ...
        eligibility_Off_SOnOff = 0.1*(psi_SOnOff*-SOnOff_R*((On_SOnOff_scale*On_SOnOff_PSC_x[:,:,:,-1] - On_SOnOff_PSC_s[:,:,:,-1]) / On_SOnOff_tauR)*(SOnOff_V[:,:,:,-1] - Off_SOnOff_ESYN)) / SOnOff_tau
        ...
        L_t_S_SOnOff_ROn = SOnOff_ROn_gSYN*(ROn_mask.astype(np.float64) - data[:,timestep,:].reshape(1,10,1))
    ...
    grads = np.sum(np.stack([grad_On_ROn], axis = 0), axis = 2)
