...
def main(trial_number,ps,scale_factor):
    ...
    R1On_Off_PSC_gSYN = ps[1]
    S1OnOff_On_PSC_gSYN = ps[2]
    R1On_S1OnOff_PSC_gSYN = ps[3]
    S1OnOff_Off_PSC_gSYN = ps[4]
    On_t_ref = ps[5]
    Off_t_ref = ps[6]
    R1On_t_ref = ps[7]
    S1OnOff_t_ref = ps[8]
    On_On_IC_g_postIC = ps[9]
    Off_Off_IC_g_postIC = ps[10]
    ...
    R1On_On_PSC_maxF = 4
    R1On_Off_PSC_ESYN = 0
    R1On_Off_PSC_tauD = 1.5
    R1On_Off_PSC_tauR = 0.7
    R1On_Off_PSC_delay = 0
    R1On_Off_PSC_fF = 0
    R1On_Off_PSC_fP = 0.1
    R1On_Off_PSC_tauF = 180
    R1On_Off_PSC_tauP = 30
    R1On_Off_PSC_maxF = 4
    ...
    R1On_Off_PSC_netcon = np.eye(Off_Npop, R1On_Npop)
    ...
    R1On_Off_PSC_s = np.ones((400,2)) * [0,0]
    R1On_Off_PSC_x = np.ones((400,2)) * [0,0]
    R1On_Off_PSC_F = np.ones((400,2)) * [1,1]
    R1On_Off_PSC_P = np.ones((400,2)) * [1,1]
    R1On_Off_PSC_q = np.ones((400,2)) * [1,1]
    ...
    Off_Off_IC_input = genPoissonInputs.gen_poisson_inputs(trial_number,Off_Off_IC_locNum,Off_Off_IC_label,Off_Off_IC_t_ref,Off_Off_IC_t_ref_rel,Off_Off_IC_rec,scale_factor,"Layer4v2")
    ...
    for t in range(0,T):
        ...
        R1On_Off_PSC_s[:,-2] = R1On_Off_PSC_s[:,-1]
        R1On_Off_PSC_s[:,-1] = R1On_Off_PSC_s[:,-1]+dt*R1On_Off_PSC_s_k1
        R1On_Off_PSC_x[:,-2] = R1On_Off_PSC_x[:,-1]
        R1On_Off_PSC_x[:,-1] = R1On_Off_PSC_x[:,-1]+dt*R1On_Off_PSC_x_k1
        R1On_Off_PSC_F[:,-2] = R1On_Off_PSC_F[:,-1]
        R1On_Off_PSC_F[:,-1] = R1On_Off_PSC_F[:,-1]+dt*R1On_Off_PSC_F_k1
        R1On_Off_PSC_P[:,-2] = R1On_Off_PSC_P[:,-1]
        R1On_Off_PSC_P[:,-1] = R1On_Off_PSC_P[:,-1]+dt*R1On_Off_PSC_P_k1
        R1On_Off_PSC_q[:,-2] = R1On_Off_PSC_q[:,-1]
        R1On_Off_PSC_q[:,-1] = R1On_Off_PSC_q[:,-1]+dt*R1On_Off_PSC_q_k1
        ...
        mask = np.any((helper[t] == (Off_tspike + R1On_Off_PSC_delay)),axis = 1)
        if np.any(mask):
            spikers = np.flatnonzero(mask) 
            R1On_Off_PSC_x[spikers,-2] = R1On_Off_PSC_x[spikers,-1]
            R1On_Off_PSC_q[spikers,-2] = R1On_Off_PSC_F[spikers,-1]
            R1On_Off_PSC_F[spikers,-2] = R1On_Off_PSC_F[spikers,-1]
            R1On_Off_PSC_P[spikers,-2] = R1On_Off_PSC_P[spikers,-1]
            R1On_Off_PSC_x[spikers,-1] = R1On_Off_PSC_x[spikers,-1] + R1On_Off_PSC_q[spikers,-1]
            R1On_Off_PSC_q[spikers,-1] = R1On_Off_PSC_F[spikers,-1] * R1On_Off_PSC_P[spikers,-1]
            R1On_Off_PSC_F[spikers,-1] = R1On_Off_PSC_F[spikers,-1] + R1On_Off_PSC_fF*(R1On_Off_PSC_maxF-R1On_Off_PSC_F[spikers,-1])
            R1On_Off_PSC_P[spikers,-1] = R1On_Off_PSC_P[spikers,-1] * (1 - R1On_Off_PSC_fP)
        ...
        dv1dOff_V_holder.append(Off_V[:,-1])
        ...
        dv1dR1On_V_holder.append(R1On_V[:,-1])
        ...
        dv1dS1OnOff_V_holder.append(S1OnOff_V[:,-1])
        ...
        dv_dR1On_Off_PSC_gSYN = np.squeeze(-(dt*R1On_R*R1On_Off_PSC_s[:,-1]*R1On_Off_PSC_netcon*(R1On_V[:,-1]-R1On_Off_PSC_ESYN)/R1On_tau)/15)
        dR1On_Off_PSC_dUk = np.squeeze(-((dt*R1On_Off_PSC_scale*2*(R1On_Off_PSC_x[:,-1]+R1On_Off_PSC_q[:,-1])/R1On_Off_PSC_tauR)*helper[t]*np.squeeze(np.sum((((Off_tspike+R1On_Off_PSC_delay)-helper[t])*np.exp(-1*((Off_tspike+R1On_Off_PSC_delay)-helper[t])**2)),axis=1)))/2500)
        dv_dR1On_Off_PSC = np.squeeze(-(dt*R1On_R*R1On_Off_PSC_gSYN*R1On_Off_PSC_netcon*(R1On_V[:,-1]-R1On_Off_PSC_ESYN)/R1On_tau)/10)
        ...
        dGSYNR1On_Off += dspike_dR1On_V*dv_dR1On_Off_PSC_gSYN
        ...
        dtref_Off = dspike_dR1On_V*dv_dR1On_Off_PSC*dR1On_Off_PSC_dUk*dspike_dOff_tref+dspike_dR1On_V*dv_dR1On_S1OnOff_PSC*dR1On_S1OnOff_PSC_dUk*dspike_dS1OnOff_V*dv_dS1OnOff_Off_PSC*dS1OnOff_Off_PSC_dUk*dspike_dOff_tref
