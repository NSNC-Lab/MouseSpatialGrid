...
def main(trial_number,ps,scale_factor):
    ...
    for t in range(0,T):
        ...
        if np.any(mask):
            spikers = np.flatnonzero(mask)
            R3On_tspike[spikers, R3On_buffer_index[spikers].astype(np.int8)-1] = helper[t]
            R3On_buffer_index[spikers] = (R3On_buffer_index[spikers] % 5) + 1
        mask = ((R3Off_V[:,-1] >= R3Off_V_thresh) & (R3Off_V[:,-2] < R3Off_V_thresh)).astype(np.int8).tolist()
        R3Off_V_spikes_holder.append(mask)
        if np.any(mask):
            spikers = np.flatnonzero(mask)
            R3Off_tspike[spikers, R3Off_buffer_index[spikers].astype(np.int8)-1] = helper[t]
            R3Off_buffer_index[spikers] = (R3Off_buffer_index[spikers] % 5) + 1
        mask = ((S3OnOff_V[:,-1] >= S3OnOff_V_thresh) & (S3OnOff_V[:,-2] < S3OnOff_V_thresh)).astype(np.int8).tolist()
        S3OnOff_V_spikes_holder.append(mask)
        if np.any(mask):
            spikers = np.flatnonzero(mask)
            S3OnOff_tspike[spikers, S3OnOff_buffer_index[spikers].astype(np.int8)-1] = helper[t]
            S3OnOff_buffer_index[spikers] = (S3OnOff_buffer_index[spikers] % 5) + 1
