...
rng(0,'twister');
n = size(Z,1);
perp = max(10, min(50, floor((n-1)/3)));
...
function [fig, ax] = make_blank_cluster_figure(method_label, S)
    ...
    tileW = 300; tileH = 500;  % <- every tile this big, regardless of K
    gapX  = 20;  gapY  = 20;
    ...
    for k = 1:K
        ...
        pad = 0.25; x = h.XData; y = h.YData;
        dx = range(x); if dx==0, dx=1; end
        dy = range(y); if dy==0, dy=1; end
        xlim(ax(k), [min(x)-pad*dx, max(x)+2*pad*dx]);
        ylim(ax(k), [min(y)-pad*dy, max(y)+2*pad*dy]);


        % 
        % ----- Reserve space on the right and park colorbar there -----
        cb_frac = 0.035;         % fraction of tile width to devote to the colorbar
        cb_gap  = 0.025;         % small gap between axes and colorbar (normalized)
        ...
        cb = colorbar(ax(k));
        cb.Units = 'normalized';
        cb.Position = [p0(1)+axw-cb_gap, p0(2)*1.1, cbw, p0(4)*0.85];  % no overlap
