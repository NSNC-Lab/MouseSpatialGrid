...
{
    double **rmgsPse, **rmgsPsi, *rmgsPsein, *rmgsPsiin, **V, **rmgse, **rmgsi, **noise;
    ...
    if (!mxIsDouble(prhs[14])|| mxIsComplex(prhs[14]) || (Nn != tempint && 1 != tempint ))
        mexErrMsgTxt("Error 15: taue must be a real double Nn vector or scalar");
    ...
    taue = scalarOrMat2Mat(prhs[14],Nn);
    ...
    for(ni=1;ni<Nn; ni++) {
        ...
        if(taue[ni] <= 0) mexErrMsgTxt("Error 15: taue <= 0");
