function newfig = figs2subplots(handles,tiling,arr,sizes)
...
% COPYLEGEND subroutine written by Ross Maddox (ross.maddox@gmail.com)
% December 2007

% RKM mod to use subplot2
...
for k = 1:min(Ns,Na)
	...
	elseif ~iscell(sizes)
		da(k) = subplot2(tiling(1),tiling(2),arr{k}, 'size', sizes);
	else
		da(k) = subplot2(tiling(1),tiling(2),arr{k}, 'size', sizes{k});
	end
	na = copyobj(av(k),newfig);
	set(na,'Position',get(da(k),'Position'));
	delete(da(k));
	if ~isempty(legend(av(k)))
		legendcopy(av(k), na);
	end
end;
