...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
    ...
    dtOtaum = dt/mxGetScalar(prhs[7]);
    Vap = mxGetScalar(prhs[8]);
    Vre = mxGetScalar(prhs[9]);
    Vth = mxGetScalar(prhs[10]);
    El = mxGetScalar(prhs[11]);
    minAPDist = max((int)(mxGetScalar(prhs[3])/dt),1);
    ...
    /* Begin integration: */
    /* taum dVdt = rmgsePse*(Ese - V) + rmgsiPsi*(Esi - V) + El - V + rmI */
    tinext = -Ns;
    if(withInhibition) {
        switch(outType) {
            case 1:
                if(Ns > 0)
                    *V = El;
                for(ti=1-Ns; ti; ti++) {
                    *++V = Vim1 + dtOtaum*(*++Pse*(Ese - Vim1) + *++Psi*(Esi - Vim1) - Vim1 + El + *++rmI );
                    if(*V < Esi)
                        *V = Esi;
                    else if(*V >= Vth) {
                        *V = Vap;
            ...
            case 2:
                for(ti=1-Ns; ti; ti++) {
                    Vim1 += dtOtaum*(*++Pse*(Ese - Vim1) + *++Psi*(Esi - Vim1) - Vim1 + El + *++rmI );
                    if(Vim1 < Esi)
                        Vim1 = Esi;
                    else if(Vim1 >= Vth) {
                        count++;
        ...
        switch(outType) {
            case 1:
                if(Ns > 0)
                    *V = El;
                for(ti=1-Ns; ti; ti++) {
                    *++V = Vim1 + dtOtaum*(*++Pse*(Ese - Vim1) - Vim1 + El + *++rmI );
                    if(*V >= Vth) {
                        *V = Vap;
                        count++;
                        tinext = min(-(1+ti),max(minAPDist,1));
            ...
            case 2:
                for(ti=1-Ns; ti; ti++) {
                    Vim1 += dtOtaum*(*++Pse*(Ese - Vim1) - Vim1 + El + *++rmI );
                    if(Vim1 >= Vth) {
                        count++;
                        tinext = min(-(1+ti),max(minAPDist,1));
                        *V++ = ti+1+Ns;
