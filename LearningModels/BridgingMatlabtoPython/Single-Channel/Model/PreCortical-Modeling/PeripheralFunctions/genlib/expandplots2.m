function expandplots2(varargin)

% EXPANDPLOTS2(FIGHANDLE, SIZE, OPTIONS)
%
...
hplots = hall(cellfun(@strcmp, types, repmat({'axes'}, size(types))) & cellfun(@strcmp, tags, repmat({''}, size(tags))));
numplots = length(hplots);
units = get(hplots, 'units');
set(hplots, 'units', figunits)
...
for ii = 1:numplots
	set(hplots(ii), 'position', newpositions(ii,:), 'units', units{ii})
end