...
s.populations(1).parameters = {'g_inc',0.01,'t_ref',1,'tau_ad',5};

s.populations(end+1).name = 'Off';
s.populations(end).equations = 'noconLIF';
s.populations(end).size = nCells;
s.populations(end).parameters = {'g_inc',0.01,'t_ref',1,'tau_ad',5};

s.populations(end+1).name='R1On';
s.populations(end).equations = 'noconLIF';
s.populations(end).size = nCells;
s.populations(end).parameters = {'g_inc',0.01,'tau_ad',5,'t_ref',1};
%s.populations(end).parameters = {'g_inc',0.0003,'tau_ad',100,'t_ref',1};

s.populations(end+1).name='S1OnOff';
s.populations(end).equations = 'noconLIF';
s.populations(end).size = nCells;
s.populations(end).parameters = {'g_inc',0.000625,'g_L',1/100,'E_L',-57,'V_reset',-52,'t_ref',0.5,'tau_ad',1}; %ginc is difference see in Differential Short-Term Plasticity of PV and SST Neurons
                                                                                                             %Accounts for Adaptation and Facilitation of Cortical
                                                                                                             %Neurons to Auditory Tones paper squared because of the units 
