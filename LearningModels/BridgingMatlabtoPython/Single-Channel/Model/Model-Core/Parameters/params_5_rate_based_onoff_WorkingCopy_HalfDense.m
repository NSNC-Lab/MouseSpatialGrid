...
%Add in dense connections (connect off path on to the on path)
%Keep gain the same for E->E, will likely have to adjust network gain to
%compensate.
% Off-> PV
varies(end+1).conxn = '(Off->R1On,R1Off->R2On)';
varies(end).param = 'gSYN';
varies(end).range = 0.02;
