...
varies(end+1).conxn = '(S1OnOff->R1On,S2OnOff->R2On,S3OnOff->R3On,S1OnOff->R1Off,S2OnOff->R2Off,S3OnOff->R3Off)';
varies(end).param = 'gSYN';
...
% On -> PV
varies(end+1).conxn = '(On->S1OnOff,R1On->S2OnOff,R2On->S3OnOff)';
varies(end).param = 'gSYN';
varies(end).range = 0.085;
%varies(end).range = [0.045:0.005:0.09];
%varies(end).range = [0.005:0.005:0.05];
%varies(end).range = [0.016:0.001:0.025];

% Off-> PV
varies(end+1).conxn = '(Off->S1OnOff,R1Off->S2OnOff,R2Off->S3OnOff)';
varies(end).param = 'gSYN';
%varies(end).range = 0.01;
varies(end).range = 0.045;
%varies(end).range = [0.005:0.001:0.014];
...
varies(end+1).conxn = '(S1OnOff,S2OnOff,S3OnOff)';
varies(end).param = 'Itonic';
varies(end).range = 0; 

varies(end+1).conxn = '(R3On->R3On)';
varies(end).param = 'FR';
varies(end).range = 8;
